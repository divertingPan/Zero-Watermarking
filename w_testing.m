image = imread('./img/lena.bmp');
seal = imread('./img/sigma.png');
seal = logical(seal);

% zero_seal = function Bit_frag_encrypt(image, seal)
...
% image = imread('./img/lena.bmp');
image = imread('./img/fake.bmp');
zero_seal = encrypt_seal;
zero_seal = logical(zero_seal);

% seal = function Bit_frag_decrypt(image, zero_seal)
image = rgb2gray(image);
image = double(imresize(image, [256,256]));
zero_seal = double(imresize(zero_seal, [256,256]));
zero_seal = zero_seal*255;
...
for i = 1:32
    for j = 1:32
        image_block = image(i*8-7:i*8, j*8-7:j*8);
        seal_block = zero_seal(i*8-7:i*8, j*8-7:j*8);
        ...
        % 5.XOR两个区块
        seal(i*8-7:i*8, j*8-7:j*8) = xor(image_block, seal_block);
